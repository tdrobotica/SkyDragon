/*Seguidor de linea digital

Ejemplo 4

Desarrollado por Skybots y tdrobotica

Objetivo:
*Hacer que el robot siga una linea usando logica digital
 Por medio de tres rangos el robot es capaz de seguir la linea.
*/
//Libreria Skybots
#include <sky.h>
//Declaracion del Objeto Robot
sky Robot;
void setup(){
  //Carga la configuraciones de I/O Linebot, Wizard, Skydragon, etc...
  Robot.Linebot();
  //Configuracion(#sen,1,2,3,4,5,6,7,8,LEDon)
  Robot.LineConfig(8, 18, 19, 15, 16, 9, 8, 7, 4, 14);
  //Espera que se presione el boton de inicio y espera
  while(Robot.Boton()){}
  //Retardo de incio
  delay(2000);
}

 void loop(){
    //Carga el valor de la posicion digital
    /*|#|#|#|#|#|#|#|#|#|#|#|#|#|#|#|#|
    Pos es una variable real con un rango 1 a 9
    y repesenta la posicion del sensor tal como
    estan marcados en el sensor de linea
    |#|#|#|#|#|#|#|#|#|#|#|#|#|#|#|#|*/
    float pos= Robot.LinePosDigital();
    //Es el rango de la izquierda
    //Gira en sentido horario
    if(pos>0 && pos<4){
      Robot.Speeds(50,50);
    }
    //Rango intermedio
    //Avanza!
     if(pos >= 4 && pos <= 5){
      Robot.Speeds(50,-50);
    }
    //Es el rango de la derecha
    //Gira en sentido anti-horario
    if(pos>5 && pos<9){
      Robot.Speeds(-50,-50);
    }
}
/*
Linebot es un robot Seguidor de lineas educativo.
contiene las siguientes conexiones:
	*Entradas
		-1 Boton D12
		-8 sensores de linea Digitales D4, D7, D8, D16, D15, D19,D18
		-1 Led-On habilitador de los sensores D14
	*Salidas
		-1 Motor MA(D5-D6)
		-1 Motor MB(D10-D11)
		-1 LED D13
	*Comunicacion
		-1 Serial Rx D0, Tx D1
		-1 I2C SDA D2, SCL D3
*/